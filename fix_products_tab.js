...
// Change updateCurrent to only update a local state if possible, or just add a "Cancel" button.
// Actually, modifying parent instantly is fine IF we just have a "Cancel" button that removes the newly added item.
// Let's see handleAddProduct:
// const newProduct = { ... };
// const updated = [...products, newProduct];
// onChange(updated);
// setEditingIdx(updated.length - 1);

// We can add a `isNew` flag or just let "Cancel" discard changes.
// It's much easier to just change handleAddProduct so it doesn't update `products` until "Save" is clicked.
// Let's introduce a `draftProduct` state.
